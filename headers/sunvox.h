...
typedef struct
{
    unsigned char	note;           /* NN: 0 - nothing; 1..127 - note num; 128 - note off; 129, 130... - see NOTECMD_xxx defines */
    unsigned char       vel;            /* VV: Velocity 1..129; 0 - default */
    unsigned char       module;         /* MM: 0 - nothing; 1..255 - module number + 1 */
    unsigned char       zero;		/* ...future use... */
    unsigned short      ctl;            /* 0xCCEE: CC: 1..127 - controller number + 1; EE - effect */
    unsigned short      ctl_val;        /* 0xXXYY: value of controller or effect */
} sunvox_note;
